TERMUX_PKG_HOMEPAGE=https://github.com/nab138/iloader
TERMUX_PKG_DESCRIPTION="User friendly sideloader. Install SideStore (or other apps) and import your pairing file with ease"
TERMUX_PKG_LICENSE="MIT"
TERMUX_PKG_MAINTAINER="@termux"
TERMUX_PKG_VERSION="1.1.5"
TERMUX_PKG_REVISION=1
TERMUX_PKG_SRCURL=git+https://github.com/nab138/iloader.git
TERMUX_PKG_DEPENDS="webkit2gtk-4.1, usbmuxd"

termux_step_make() {
	termux_setup_rust
	termux_setup_nodejs

	cd $TERMUX_PKG_SRCDIR/src-tauri
	cargo clean
	cargo vendor vendor/

	# Needed so we can patch the hardcoded usbmuxd socket path
	# Has to be below the cargo vendor command or it won't be able to fetch it
	cd $TERMUX_PKG_SRCDIR
	patch -N -p1 -i $TERMUX_PKG_BUILDER_DIR/0001-Include-idevice-in-patching.diff
	cd $TERMUX_PKG_SRCDIR/src-tauri

	echo "" >> Cargo.toml
	echo '[patch.crates-io]' >> Cargo.toml
	crates_to_patch=(
		softbuffer
		tao
		tauri-macros
		tauri-plugin-dialog
		tauri-plugin-fs
		tauri-plugin-opener
		tauri-runtime-wry
		tauri-runtime
		tauri-utils
		tauri
		tauri-plugin
		wry
		muda
		rfd
		idevice
		zsign-rust
	)
	# idevice needs further patching but the patch earlier already overrides the version with
	# the path to the patched dir, so we skip this so it doesn't error with duplicate tokens
	crates_to_skip_cargo_toml=(
		idevice
	)

	# # If there is an issue with the automated patching, uncomment these to make it save the patches in src-tauri/patches. You can simply git am this dir.
	# git add -A && git commit -m "add vendored folders" && git tag -d unpatched
	# git tag unpatched
	for crate in "${crates_to_patch[@]}"; do
			skip=false
			echo "termuxifying '$crate'..."
			find "vendor/$crate" -type f | \
					xargs -n 1 sed -i \
					-e 's|"android"|"disabling_this_because_it_is_for_building_an_apk"|g' \
					-e 's|"linux"|"android"|g' \
					-e "s|libxkbcommon.so.0|libxkbcommon.so|g" \
					-e "s|libxkbcommon-x11.so.0|libxkbcommon-x11.so|g" \
					-e "s|libxcb.so.1|libxcb.so|g" \
					-e "s|/tmp/|$TERMUX_PREFIX/tmp/|g" \
					-e "s|/var/|$TERMUX_PREFIX/var/|g"

			# We can't use /tmp cause there are files that only start with that, but zsign-rust does only that to set the tmp folder.
			if [[ "$crate" == "zsign-rust" ]]; then
				find "vendor/$crate" -type f | \
						xargs -n 1 sed -i \
						-e "s|s_strTempFolder = \"/tmp\";|s_strTempFolder = \"$TERMUX_PREFIX/tmp\";|g"
			fi

			for skip_crate in "${crates_to_skip_cargo_toml[@]}"; do
					if [[ "$crate" == "$skip_crate" ]]; then
							skip=true
							echo "skipping Cargo.toml entry of '$skip_crate'..."
							break
					fi
			done
			if $skip; then
					continue
			fi
			echo "$crate = { path = \"./vendor/$crate\" }" >> Cargo.toml

			# git add -A && git commit -m "automated termux patch for $crate"
	done
	# git format-patch unpatched -o patches

	# Generates the dist folder. Required else build fails.
	npm install && npm run build
	# Apply patch that allows arm and i686 to successfully build
	# TODO: This patch is already merged into master. We simply need to update idevice when a release is made with it.
	cd $TERMUX_PKG_SRCDIR/src-tauri/vendor
	patch -N -p1 -i $TERMUX_PKG_BUILDER_DIR/0001-Fix-failure-to-compile-to-32-bit.diff
	cd $TERMUX_PKG_SRCDIR/src-tauri
	echo "idevice = { path = \"./vendor/idevice\" }" >> Cargo.toml
	# OpenSSL doesn't declare ERR_print_error_fp without this which is used by zsign-rust
	export OPENSSL_NO_VENDOR=1
	if [ "$TERMUX_DEBUG_BUILD" = "true" ]; then
		cargo build --bins --features tauri/custom-protocol --target $CARGO_TARGET_NAME
	else
		cargo build --bins --features tauri/custom-protocol --release --target $CARGO_TARGET_NAME
	fi
}

termux_step_make_install() {
	if [ "$TERMUX_DEBUG_BUILD" = "true" ]; then
		install -Dm700 -t $TERMUX_PREFIX/bin $TERMUX_PKG_SRCDIR/src-tauri/target/$CARGO_TARGET_NAME/debug/iloader
	else
		install -Dm700 -t $TERMUX_PREFIX/bin $TERMUX_PKG_SRCDIR/src-tauri/target/$CARGO_TARGET_NAME/release/iloader
	fi
}
