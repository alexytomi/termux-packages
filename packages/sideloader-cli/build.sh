...
_COMMIT_VERSION="35f486c"
TERMUX_PKG_VERSION="1.0-pre4.${_COMMIT_VERSION}"
TERMUX_PKG_DEPENDS="usbmuxd, libplist, libimobiledevice"
TERMUX_PKG_BUILD_DEPENDS="ldc, ndk-sysroot, jq"
TERMUX_PKG_SRCURL=https://github.com/Dadoum/Sideloader/archive/${_COMMIT_VERSION}.zip
...
termux_step_configure() {
	...
	patch -N -p1 -i $TERMUX_PKG_BUILDER_DIR/0001-Provision-Remove-glibc-backtrace-call.diff || true # It might already be applied
	cd ..
	# Patch plist-d
	git init libplist-d
	cd libplist-d
	...
	git checkout FETCH_HEAD
	patch -N -p1 -i $TERMUX_PKG_BUILDER_DIR/0001-plist-d-Add-filename-for-termux-libplist-2.0.so.diff || true # It might already be applied
	cd ..
}
